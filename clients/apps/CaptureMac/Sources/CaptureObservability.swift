...
enum CaptureObservability {
    private static let logger = Logger(subsystem: "dev.crmitchelmore.capture.mac", category: "observability")
    ...
    static func start() {
        ...
        wideEvent("native.observability.started", fields: ["surface": "macos"])
    }
    ...
    static func wideEvent(_ event: String, fields: [String: String] = [:]) {
        CaptureDiagnostics.record(
            ...
            name: event,
            message: event,
            fields: fields.merging(["service": "capture-mac"]) { first, _ in first }
        )
    }
    ...
    private static func emit(_ event: CaptureDiagnosticEvent) {
        ...
        payload["service"] = payload["service"] ?? "capture-mac"
