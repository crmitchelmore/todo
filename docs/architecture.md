...
Postgres + PowerSync** backend on the selected Mac mini rather than iCloud/CloudKit. This:
 ...
 Capture &  │  (UIKit)        (AppKit)                          (React)             │
   sync     │     │  Share Sheet / Siri / Shortcuts / widget / hotkey               │
            ...
            └────────────────────┼───────────────┼─────────────────────────────────┘
                          ...
                          ┌──────▼───────────────▼──────┐
            Sync core     │  Postgres + PowerSync service │  (Mac mini)
                          └──────▲───────────────▲──────┘
...
> only — it never changes `status`. It runs natively on the selected backend Mac against the shared
> Postgres so approved local-harness work can execute on that machine. The on-device pass writes `suggestion_source='on-device'`;
