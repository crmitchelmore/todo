...
- **Team ID**: `8X4ZN58TYH`
- **API key ID**: `SB8RA7B6WG` → `~/.private_keys/AuthKey_SB8RA7B6WG.p8`
...
```
Defaults: key `SB8RA7B6WG`, team `8X4ZN58TYH`, version from `./VERSION`, build = unix timestamp.
...
- `APP_STORE_CONNECT_KEY_ID` — `SB8RA7B6WG`
- `APP_STORE_CONNECT_ISSUER_ID` — issuer UUID
- `APPLE_TEAM_ID` — `8X4ZN58TYH`

Set the base64 key with:
```bash
gh secret set APP_STORE_CONNECT_API_KEY --body "$(base64 -i ~/.private_keys/AuthKey_SB8RA7B6WG.p8)"
gh secret set APP_STORE_CONNECT_KEY_ID --body SB8RA7B6WG
